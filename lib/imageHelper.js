...
export const getWatchImage = (watchId, originalImage = null) => {
    ...
    if (originalImage && typeof originalImage === 'string') {
        // Use original image if it's a valid URL (external)
        if (originalImage.startsWith('http')) {
            return originalImage;
        }

        // For local images, always flatten the path to root since all images are in public/
        // This handles legacy paths like '/TimeWise Watch Designs/Men/Watch.jpg'
        // and ensures they resolve to '/Watch.jpg'
